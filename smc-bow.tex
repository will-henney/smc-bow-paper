...
\usepackage{gensymb}
%\usepackage{microtype} % Activate for final version maybe
\usepackage{savesym}
\savesymbol{tablenum}
\usepackage{hyperref}
...
\newcolumntype{C}{>{$}c<{$}} 
\usepackage{ragged2e}
% define a raggedright column with hyphenation support
...
% WJH 2025-08-25 : Try to tweak the figure placement.
% - Try to allow some float-only pages so that the figures end up closer to the text
% that describes them
\renewcommand{\topfraction}{0.7}    % max fraction of floats at top 
\renewcommand{\textfraction}{0.2}   % min fraction of text in column
...
\begin{figure}
  \centering
  \includegraphics[width=\linewidth]{figs/ngc346-fors1-profiles-annotated}
  \caption{
...
JWST observations of NGC~346 \citep{Habel:2024a} also clearly show the bow shock
at wavelengths of \qtyrange{15}{21}{\micron}. They also show several other smaller bow shocks, for instance around SSN~13 ans SSN~31. Then there is a big bow shock that is associated with HD~5980 (the WR+WR binary).
...
\begin{figure}
  \centering
  \includegraphics[width=\linewidth]{figs/ngc346-bow-shock-ariv-new-diagnostics-annotated}
  \caption{
...
\begin{figure}
  \centering
  \includegraphics[width=\linewidth]{figs/ngc346-infrared-spitzer-spectra-annotated}
  \caption{
    Mid-infrared Spitzer spectra of (a) background-subtracted bow shock,
    (b)~background-subtracted mYSO~C,
    (c)~nebula background emission. 
    }
  \label{fig:infrared-sed}
\end{figure}
